<template>
    <div class="relative" :class="fieldClass">
      <button type="submit" 
        ...
        :aria-busy="loading"
        aria-live="polite">
        <fa v-if="loading" :icon="['fas', 'spinner']" class="animate-spin mt-0.5 h-5 w-5 mr-2" aria-hidden="true"/>
        <fa v-else :icon="['fas', iconName]" class="mr-2 mt-0.5" aria-hidden="true"/>
        {{ label }}
      </button>
